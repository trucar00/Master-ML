...
df["del_cog"] = df.groupby("mmsi")["cog_interp"].diff().apply(angle_wrap)
...
for traj_id, idx in tqdm(df.groupby("trajectory_id").groups.items()):
    ...
    d = df.loc[idx].sort_values("datetime")  # keep original index
    ...
    while current + window_length <= end:
        ...
        in_window = (d["datetime"] >= current) & (d["datetime"] < current + window_length)
        ...
        window_df_clean = window_df_all.dropna(subset=["lon", "lat", "speed", "cog_interp"])
